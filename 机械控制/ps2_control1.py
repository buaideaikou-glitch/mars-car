...
line_turn_cooldown = False  # 转弯冷却（防止重复触发）
line_turn_cooldown_time = 0  # 冷却开始时间
_LINE_TRACK_SPEED = 0.5    # 循迹前进速度 (rad/s)
_LINE_TRACK_MAX_STEER = 90.0  # 循迹最大转向角 (deg)
_LINE_TRACK_DX_THRESHOLD = 15  # dx阈值，小于此值直行
_LINE_TRACK_LOST_TIMEOUT = 3.0  # 丢失超时自动退出 (秒)
_LINE_PIVOT_SPEED = 1.5    # 90度转弯pivot速度 (rad/s)
_LINE_PIVOT_90_MS = 1500   # 90度转弯持续时间 (ms，需实测调整)
_LINE_TURN_COOLDOWN_SEC = 2.0  # 转弯后冷却时间 (秒)
_LINE_TURN_SEARCH_SEC = 3.0    # 转弯后搜索新线条超时 (秒)
...
def ps2_loop(rover, ps2, data, serial):
    ...
    global line_track_mode, line_track_lost, line_track_lost_time, line_turn_cooldown, line_turn_cooldown_time
    ...
    while True:
        ...
        if button_pressed(buttons, ps2.PS2_BTN_CROSS):
            ...
            if line_track_mode:
                ...
                line_turn_cooldown = False
                line_turn_cooldown_time = 0
        ...
        # 自动循迹模式的控制逻辑
        # 协议: <dx> <status>
        #   status=0: 循迹中, dx=线条偏移
        #   status=1: 左转90°   status=2: 右转90°   status=3: 丢失
        # ==========================================================
        if line_track_mode:
            # 转弯冷却检查
            if line_turn_cooldown and time.time() - line_turn_cooldown_time > _LINE_TURN_COOLDOWN_SEC:
                line_turn_cooldown = False
                print("✅ 转弯冷却结束")
            ...
            if serial_data is not None:
                ...
                if len(parts) == 2:
                    try:
                        ...
                        # ===== 90度转弯 =====
                        if status == 1 and not line_turn_cooldown:
                            print("↰ 检测到左转，执行90度旋转")
                            rover.stop()
                            time.sleep_ms(300)
                            rover.pivot_turn(speed_rad_s=-_LINE_PIVOT_SPEED)
                            time.sleep_ms(_LINE_PIVOT_90_MS)
                            rover.stop()
                            time.sleep_ms(300)
                            # 搜索阶段：缓慢同向旋转直到K210找到新线条
                            data["value"] = None
                            search_start = time.time()
                            while time.time() - search_start < _LINE_TURN_SEARCH_SEC:
                                s_data = data["value"]
                                if s_data is not None:
                                    data["value"] = None
                                    s_parts = s_data.strip().split()
                                    if len(s_parts) == 2:
                                        try:
                                            s_dx = int(s_parts[0])
                                            s_status = int(s_parts[1])
                                            if s_status == 0 and abs(s_dx) < 120:
                                                print("✅ 左转完成，已找到新线条 dx={}".format(s_dx))
                                                break
                                        except:
                                            pass
                                rover.pivot_turn(speed_rad_s=-_LINE_PIVOT_SPEED * 0.4)
                                time.sleep_ms(100)
                                rover.stop()
                                time.sleep_ms(50)
                            rover.stop()
                            line_turn_cooldown = True
                            line_turn_cooldown_time = time.time()
                            print("✅ 左转完成")

                        elif status == 2 and not line_turn_cooldown:
                            print("↱ 检测到右转，执行90度旋转")
                            rover.stop()
                            time.sleep_ms(300)
                            rover.pivot_turn(speed_rad_s=_LINE_PIVOT_SPEED)
                                ...
                                if s_data is not None:
                                    ...
                                    if len(s_parts) == 2:
                                        try:
                                            ...
                                            s_status = int(s_parts[1])
                                            if s_status == 0 and abs(s_dx) < 120:
                                                print("✅ 右转完成，已找到新线条 dx={}".format(s_dx))
                                                break
                                        except:
                                            pass
                                rover.pivot_turn(speed_rad_s=_LINE_PIVOT_SPEED * 0.4)
                        ...
                        # ===== 正常循迹 =====
                            ...
                            # P控制：dx>0线在右侧右转，dx<0线在左侧左转
                            steer_angle = 0.0
                            if abs(dx) > _LINE_TRACK_DX_THRESHOLD:
                                steer_ratio = clamp(dx / 320.0, -1.0, 1.0)
                            ...
                            if abs(dx) > 50:
                                print(f"🟢 循迹: dx={dx:+d} → 转向={steer_angle:+.1f}° 速度={speed:.2f}")
